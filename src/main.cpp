...
void sendSliderValuesToClient(AsyncWebSocketClient* client) {
  ...
  while (i < NUMSLIDERS && (millis() - start < 50)) { // Timeout: 50ms
    if (Wire.available()) {
  ...
  if (type == WS_EVT_CONNECT) {
    ...
    xTaskCreate(
      [](void* param) {
        ...
        delay(1000); // Wait for the frontend to initialize
        sendSliderValuesToClient(c); // Read from slave and send to client
        vTaskDelete(NULL); // End task
      },
      "InitSliderSender", // Name of the task
      4096,               // Stack size
      (void*)targetClient,// Parameter to task
      1,                  // Priority
...
void loop() {
  // No polling loop required anymore
}